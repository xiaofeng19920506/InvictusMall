            ...
            <div key={order.id} className="bg-white rounded-lg shadow-md p-6">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                <div>
                  ...
                  {order.shippedDate && (
                    <p className="text-sm text-gray-600">Shipped on {formatDate(order.shippedDate)}</p>
                  )}
                  {order.deliveredDate && (
                    <p className="text-sm text-gray-600">Delivered on {formatDate(order.deliveredDate)}</p>
                  )}
                </div>
                  ...
                  <div className="space-y-1">
                    {order.items.map(item => (
                      <div key={item.id} className="flex items-center justify-between text-sm">
                        ...
                        <p className="text-gray-900 font-medium">${item.subtotal.toFixed(2)}</p>
                      </div>
                    ))}
                  </div>
                </div>
